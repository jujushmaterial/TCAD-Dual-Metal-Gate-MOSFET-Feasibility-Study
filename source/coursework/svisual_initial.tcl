...
foreach {name value} [list \
  Vth_Low $Vth_Low Vth_High $Vth_High \
  Vtgm_Low $Vtgm_Low Vtgm_High $Vtgm_High \
  DIBL_VperV $DIBL_VperV DIBL_mVperV $DIBL_mVperV \
  SS_Low $SS_Low SS_High $SS_High \
  gm_Low $gm_Low gm_High $gm_High \
  Ion_Low $Ion_Low Ion_High $Ion_High \
  Ioff_Low $Ioff_Low Ioff_High $Ioff_High \
  IonIoff_Low $IonIoff_Low IonIoff_High $IonIoff_High \
  IdMax_Low $IdMax_Low IdMax_High $IdMax_High] {
  wb_scalar $name $value
}

if {[info exists runVisualizerNodesTogether]} {
  set_curve_prop IdVg_Low($n) -label "Low Vd IdVg $legend" -color $color -line_style $line -line_width 3
  set_curve_prop IdVg_High($n) -label "High Vd IdVg $legend" -color $color -line_style dashed -line_width 3
}
